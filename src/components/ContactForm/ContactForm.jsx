...
export default function ContactForm() {
	...
	const [name, setName] = useState({value: "", isValid: false, errorMessage: "minimum required letters: 3"});
	const [email, setEmail] = useState({value: "", isValid: false, errorMessage: "invalid email"});
	const [message, setMessage] = useState( {value: "", isValid: false, errorMessage:"this field can not be empty"});
				...
				<label htmlFor='name'>
					...
					<div className={styles.inputGroup}>
						...
						<input
							...
							aria-label='name'
							value={name.value}
							onChange={(event) => setName({...name, value: event.target.value, isValid: isValidName(event.target.value)} )}
						/>
					</div>
					{!name.isValid && submitStatus !== statusValues.initial && 
					<div className={styles.errorMessage}>{name.errorMessage}</div>
					}
				</label>
				<label htmlFor='email'>
					Email*:
					<div className={styles.inputGroup}>
						...
						<input
							...
							aria-label='email'
							value={email.value}
							onChange={(event) => setEmail({...email, value: event.target.value, isValid: isValidEmail(event.target.value)})}
						/>
					</div>
					{!email.isValid && submitStatus !== statusValues.initial &&
				...
				<label htmlFor='name'>
					...
					<div className={styles.inputGroup}>
						...
						<textarea
							...
							aria-label='message'
							value={message.value}
							onChange={(event) => setMessage({...message, value: event.target.value, isValid: isValidMessage(event.target.value)})}							/>
					</div>
					{!message.isValid && submitStatus !== statusValues.initial &&
